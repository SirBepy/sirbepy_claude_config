...
- Before recommending moving anything to/from a drive letter, check its physical media type first: read the vault note `Dev machine storage.md` for drives already confirmed there; for any drive not yet in it, run `Get-PhysicalDisk | Select DeviceId, FriendlyName, MediaType` + `Get-Partition -DriveLetter <X>`, then add it to the vault note. Same physical disk under a different partition carries no penalty - a genuinely different physical disk does.
- If either side of a cross-drive move lands on an HDD, name the tradeoff explicitly: load-time increase, mid-play streaming stutter/pop-in for asset-heavy titles, contention with anything already writing to that disk (e.g. `D:\cargo-target` builds), audible seek noise. SSD-to-SSD moves need none of this caveat.

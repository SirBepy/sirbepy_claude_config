...
6. **Show it to the dev.**
   - **Standalone-file branch:** open the file directly in the dev's default browser so it always shows up reliably - PowerShell `Start-Process "<abs-path-to-file>"` (Joe 2026-07-24: "mockup should host the website always"; the in-app Conductor preview panel is too fragile/buried - it only auto-opens off a lossy live nudge and otherwise hides in the Chats window's `⋮` menu, so a pushed preview silently never appears). Then also capture a screenshot via `SendUserFile` so the dev has it in-chat too. `/preview` (panel push) is still available if the dev explicitly asks for the docked panel, but it is no longer the default. This reverses the 2026-07-20 panel-first decision.

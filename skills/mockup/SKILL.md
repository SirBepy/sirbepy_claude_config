...
5. **Verify before showing the dev - actually look, don't assume the wiring worked.** Fetch/render the page and READ BACK a screenshot yourself before calling it ready. Confirm the real classes/CSS actually applied (a card should look like a card - bordered, backgrounded, padded - not bare unstyled text) and nothing silently failed to import/scope. A preview that compiles without error is not the same as a preview that rendered correctly - this is a distinct, non-skippable check from step 6's "capture a screenshot for the dev," which happens after this one passes, not instead of it.
   ...
   Eyeballing the screenshot isn't enough on its own - a gated/compound selector (`.ancestor .foo`, `#some-id .foo`, or a rule depending on a custom property defined elsewhere) can silently fail to match while a component's children still look plausible (their own borders/spacing survive even when the parent's chrome doesn't apply - three separate incidents have each passed a glance-check this way). Before trusting the screenshot: grep the reused stylesheet(s) for the selectors being copied, and for any gated one either satisfy the gate in the preview markup/imports or note in the mockup's header comment what was hand-copied instead. Then confirm with a computed-style check, not just a look - add an `evaluate` step reading `getComputedStyle(el)` (background/border/color, etc.) for at least one reused element and compare it against the stylesheet's actual rule.

   If the mockup makes a falsifiable geometric or numeric claim about the render itself - square, centered, aligned, N px, same width - MEASURE it, don't eyeball it. Add an `evaluate` step calling `getBoundingClientRect()` on the relevant element(s) and compare the actual numbers against the claim before stating it as fact. A screenshot is evidence the page rendered, not that the specific claimed property is true: one incident asserted "the portrait is now a true square at every density" across three rounds and passed every glance-check, while the element measured 46x59 the whole time because two CSS-cascade bugs meant the rule never applied. This is a distinct check from the render/computed-style check above - that one catches unstyled div soup, this one catches a mockup that renders plausibly while the specific claimed property is false.
   ...
   - Step JSON shape per type: `{"type":"click","selector":"..."}`, `{"type":"hover","selector":"..."}` (real `:hover` CSS - a dispatched `mouseover` event does NOT trigger it), `{"type":"selectOption","selector":"...","value":"..."}`, `{"type":"wait","ms":N}`, `{"type":"scroll","to":N}`, `{"type":"screenshot","out":"path.png"}` (field is `out`, not `path`), `{"type":"waitForSelector","selector":"...","timeout":N}` (timeout optional, default 10000), `{"type":"refresh"}`, `{"type":"evaluate","js":"..."}` (field is `js`, not `script`). `evaluate`'s `js` string is passed straight to `page.evaluate(step.js)` as code to execute, not a function reference to call - it must be a self-invoking IIFE (`(function(){...})()` or `(() => {...})()`); a bare `() => {...}` evaluates to an inert function object and silently does nothing.

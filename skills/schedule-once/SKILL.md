...
- Logs persist at `%LOCALAPPDATA%\ClaudeScheduleOnce\logs\<task>.log` even after the task self-deletes, so Joe can review what happened. Each run prunes logs older than ~30 days so the folder doesn't grow forever.

...
8. **Commit by pathspec, never stage-then-commit.** Five preconditions, checked right here, not skimmed past earlier - all required, every single commit, not once per run:
   ...
   - **Unpushed-overlap check (hunk-level, not file-level - see todo 368, script per todo 474):** `bash ~/.claude/skills/commit/overlap-check.sh -C <repo> <files>` (absolute path; same pathspec as this commit). Runtime scales with pathspec size times unpushed-commit depth - whenever the pathspec exceeds roughly a dozen files, give this call an explicit `timeout` (up to 600000ms); the Bash tool's 120s default otherwise auto-backgrounds it, and a backgrounded gate is an unrun gate that must complete before `git commit` runs. The script owns the algorithm end to end - file-level pre-filter against `@{u}..HEAD`, then per-file hunk-range blame, comparing full 40-char shas throughout so a candidate sha can never be silently missed by width or a boundary-commit `^` marker. Exit 0 = clean: no upstream, or every file-level candidate cleared as line-disjoint (each printed as its own one-line "unrelated commit, no shared lines" - the common, expected case todo 368's hunk-level upgrade exists for) - proceed without asking. Exit 2 = the script could not run - a plain-English `ERROR:` line, fix the path/repo and rerun, not a finding to act on. Exit 1 = at least one real hunk-level hit, printed as `<file>:<range> <sha> <subject>` - the decision on a hit is policy, not algorithm, and lives here only. Before choosing which card to ask, check whether the repo's `.claude/commit-style.md` (already read in step 1) forbids sharing a commit across tickets or units of work, and whether any blamed commit belongs to a different ticket than this one - if both hold, that rules out every fold on this hit, HEAD or non-HEAD, this session's commit or a peer's: skip `AskUserQuestion` entirely, take the genuinely-separate branch, and print one line naming the overlapping commit, the blamed lines, and the quoted rule that ruled out the fold (todo 849). Absent that, check whether any blamed sha IS the current `HEAD`: **interactive session, overlap includes HEAD** - STOP, name the overlapping commit and the blamed lines, ask via `AskUserQuestion` whether this is follow-up on the same unit of work (-> `git reset --soft HEAD~1`, restage everything together, one fresh commit) or genuinely separate (-> proceed). **interactive session, overlap is entirely non-HEAD** - `reset --soft HEAD~1` only folds into the immediately preceding commit, so it cannot fold these; do not offer it. STOP, name the overlapping commits and blamed lines, ask via `AskUserQuestion` whether to fold via `/commit fold <sha>` (recipe above) or state it and proceed. **Unattended run** (`/auto-do-todos`, `/autopilot`, or any caller that cannot block on a question) - there is no one to ask, so take the genuinely-separate branch and proceed, but record the overlapping commit, file, and blamed lines in that run's own summary/report-back; a human reviews it after the fact instead of blocking the run. This is the one place that unattended-hit behavior is defined - it is not restated in the runner skills themselves.

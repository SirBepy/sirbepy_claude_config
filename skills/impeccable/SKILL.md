...
- The screenshot round is non-skippable when a mockup was the spec: green typecheck/test/build cannot detect a UI that doesn't match the approved mockup, so a build thread may not quietly omit it. If plain headless Chromium renders nothing, don't conclude rendering is impossible here: launch with an isolated `--user-data-dir` pointed at a throwaway temp profile, and use `--dump-dom` to confirm what markup is actually live before trusting the screenshot. (Table Night redesign, 2026-08-03: this exact false conclusion removed the only check that would have caught a stale-dependency shadow shipping the wrong UI - see `~/.claude/refs/delegation-doctrine.md`'s "Visual work" rule.)

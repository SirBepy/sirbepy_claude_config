---
name: setup-flutter-cicd
description: Alias for /flutter-cicd - sets up a Flutter project's Android release pipeline (signing keystore, signed APK to GitHub Releases, optional Google Play internal-track upload, optional GitHub Pages web deploy).
disable-model-invocation: true
---

# /setup-flutter-cicd

> Alias. All content lives in `/flutter-cicd` so the two can never drift.

Read `~/.claude/skills/flutter-cicd/SKILL.md` in full and follow it exactly. Do not
reimplement or summarize any of its steps here - this file is a pointer, nothing else.

...
- **A todo belongs in the backlog of the repo it changes.** A finding about the global `~/.claude` tree (a skill, a global rule, a hook, `CLAUDE.md` itself) goes in `~/.claude/todos/`, NEVER in a project's `.claude/todos/` - even when the project session is what surfaced it. Write it there and move on; do not queue it locally "for later".

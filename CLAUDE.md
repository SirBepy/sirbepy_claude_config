...
- Before asserting "X does/causes Y because Z" about a system not read or run this session: read it first, even one file. If you can't check right now, write "UNVERIFIED: <claim>, would check <file/log>" instead of stating it as fact - hedging the noun ("suspect") does not hedge a trailing "so Y happens" clause, the whole sentence needs the label.

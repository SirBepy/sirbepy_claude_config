...
- Done 2026-08-26: all five Approach items. Step 5 and the Precedence line both now say the run never pushes or deploys on its own initiative; plain git push is in the Hard Stops list with the already-authorised exception written down inline; snippets/auto-commit.md opens with a scope sentence saying it governs committing only and grants nothing about pushing; /delegate checked and has zero push mentions, so nothing to fix there. Item 2's oracle constraint went in separately: the oracle may only restate criteria the request contained, may never add an ACTION, and meeting a self-written criterion is not authorisation. Dispatch note for future runs: the orchestrator paraphrased this todo into a 4-item task list and silently dropped item 2; the builder caught and reported the omission rather than quietly closing around it, which is the only reason it got done.

...
<!-- cleanup: last-checked 2026-08-15, complexity=EASY, worth=7, reconfirm-count=1, content-hash=e334c431 -->
...
- Completed via /auto-do-todos 2026-08-15: added hooks/todos-em-dash-guard.py, a PreToolUse hook on Write/Edit/MultiEdit scoped to .claude/todos/ by exact path-segment match (never substring), using the same exact chr(0x2014) codepoint check as em-dash-guard.py. BLOCKS rather than auto-fixes: the writing session can act on the message immediately, and a silent substitution would corrupt the legitimate case of a todo quoting U+2014 as its own subject. Exemption marker <!-- em-dash-exempt --> is honoured from the on-disk text too, so it survives later Edits. New test suite uses hooks/_testlib.py; settings.json re-verified to parse with no BOM.

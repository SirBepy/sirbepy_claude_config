...
<!-- cleanup: last-checked 2026-09-01, complexity=EASY, worth=3, reconfirm-count=2, content-hash=eadc5677 -->
<!-- duplicate-checked -->
...
- Deduped the prefix-less-stem fallback into the new skills/close/_shared.ps1, dot-sourced by claim-todo.ps1 and complete-todo.ps1 (c68963b). The first builder was killed by the session limit having done only claim-todo.ps1; the retry read and judged that partial rather than assuming it correct, then finished complete-todo.ps1. The asymmetry is preserved by returning the matched Pattern from the helper so complete-todo.ps1 can reuse it for its done/ search. Two findings filed rather than fixed: 878 (the -Id batch example in the comment-based help is unrunnable as written) and 882 (a prefix-less todo already in done/ reports as missing, pre-existing and confirmed present before this refactor).

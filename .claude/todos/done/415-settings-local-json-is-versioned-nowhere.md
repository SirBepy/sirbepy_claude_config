...
- Done 2026-08-20, option 1 per Joe's call on the two-option card: durable config moved into the tracked settings.json, settings.local.json stays deliberately untracked. Moved: the impeccable design-detector wiring (PostToolUse on Edit|Write|MultiEdit, 5s; plus the full-rule deep pass on Stop, 30s), the status-marker-guard.py Stop hook (which was wired in NO other file), outputStyle Silent, and 6 permission allows. Command strings copied verbatim, including their 4-backslash JSON escaping, so nothing about how they execute changed. settings.local.json is now a single self-documenting description key saying it is machine-local by design and that Claude Code auto-appends 'always allow' grants there. Nothing secret was present, so no .gitignore exception was needed and no new file became tracked. PROOF both moved hooks are still wired, from real nested claude -p runs whose config dir (.claude-fibo) symlinks settings.json and settings.local.json to this repo's copies: (1) status-marker-guard - prompted for a literal malformed marker, the run answered 'Hook rejects `bogus` as a status value, so that exact output can't stand' and corrected to a valid marker, so the Stop hook blocked from its new home; (2) impeccable - a real .css write produced '[impeccable@1] Design hook scanned card.css. No deterministic design-quality issues found.' as PostToolUse additional context. Scratch dir deleted after. python ci/run_all.py exits 0.

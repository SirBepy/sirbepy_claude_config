...
- **2026-08-19, recurrence (revaire-mobile REV-5312, PR #969).** Joe wrote, in the same message that queued the command: *"when you do end up doing /create-pr auto approve creating, dont show me no draft, go straight to actually making the PR"*. The session skipped the gate and disclosed the override in its report. Note this is NOT the shape option 2 was written against: it was current and specific to this PR, not an earlier blanket instruction reused later. But step 5's other clause ("not skippable even when invoked as the tail of a bundled instruction") does cover it, so the rule was broken either way. Still one open question for Joe: does a current, specific, same-message override count, or is the gate absolute against him too? Reopen this todo if he wants the carve-out; leave closed if he wants the gate to hold against his own instruction.

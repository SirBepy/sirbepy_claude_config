...
- Completed via /auto-do-todos 2026-08-15: added an explicit Attended mode section, scoped the Precedence supersede clause to unattended runs only, and split Steps 5, 6 and 7 never-ask promises into unattended (unchanged) and attended (may ask inline) branches. Trigger is invitation phrasing in the invoking prompt, detected once at Step 1, mirroring the existing Cleanout intent phrase-detection prior art; a mid-run "did the dev reply" signal was rejected because a run cannot decide Step 5 retroactively. The builder also introduced a 25-file size threshold that would have SKIPPED Steps 2-3 entirely on small backlogs; the orchestrator replaced it, since /cleanup-todos is the only pass that dedupes, re-verifies and archives, and Step 4 triage does none of those. Steps 2-3 now carry an honest explicit condition instead: Step 2 always runs, Step 3 is skipped only when Step 2 leaves nothing EASY.

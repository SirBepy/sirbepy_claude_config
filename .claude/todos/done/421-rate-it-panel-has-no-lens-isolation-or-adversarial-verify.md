...
- Shipped 2026-08-22. Lens isolation was ALREADY DONE before this todo (commit fa3dcf8 gave each rater one of four fixed lenses), so that acceptance line needed no work and the todo overstated the gap. Real work: an adversarial verification pass in panel.md between fan-out and synthesis, batched into at most 3 verifier dispatches by default with a `strict` token for one-per-flaw (Joe chose the flag shape; the expensive variant is NOT known to be better, its only mechanism is removing cross-finding anchoring and that effect is unmeasured). Verifiers see only the hypothesis and their own assigned claims, never a score, a lens name, a rater count or the synthesis, which does not exist yet. Outcomes confirmed/refuted/unverifiable are distinct, only confirmed claims feed the score and lifts, unverifiable is reported on its own line and never averaged in. Added a STAND/REVISE/ESCALATE verdict line; the numeric score line is untouched and still line 1, so /rate-it-and-commit parses as before (asserted by fixture 9 expectation 1). Measured before/after on the SAME hypothesis via todo 422 harness, fixture 9: OLD panel 3/6, and its own words were "Unanimous panel: all three lens raters and my own independent pass landed on 3/10", resting on a Lambda 15-minute-ceiling claim. NEW panel 6/6, refuting five claims by name including that exact ceiling claim (Fargate-backed states have no such limit), with the main agent dissenting at 6/10. Honest split: the SCORE did not move (3/10 both times), what moved is the evidence base and the recorded dissent, so the groupthink half is demonstrated and the score-mobility half is not. First live run also exposed a real defect in this very change, a lift resting on a claim the same report called refuted; fixed with an explicit consistency check in synthesis step 5 and re-measured 5/6 to 6/6, verdict won. Fixture 6 held at 4/4 across the whole change, so no regression. Panel runs are expensive: about 2 USD per panel fixture, 6 subagents each.

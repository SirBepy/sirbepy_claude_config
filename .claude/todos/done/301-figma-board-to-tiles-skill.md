...
- Done 2026-08-13 as skills/figma-tiles/, with the quota-critical machinery factored into skills/_shared/figma_client.py so it is written once. Three subcommands: sweep (tree fetch, per-screen /v1/images render, comment digest, manifest.json in one command), slice (OFFLINE fallback using the generic colour-mask recursive-split algorithm, zero API calls), and comments (standalone digest). Ported read-only from zng-app's .for_bepy/figma/ scripts; nothing in zng-app was modified. Quota handling is enforced by the tool rather than left to memory, which was the todo's own acceptance bar: depth is hard-capped at 3 in code (MAX_SAFE_DEPTH, raises FigmaQuotaError without an explicit force), there is NO whole-file fetch path at all, every fetch is disk-cached so a cache hit costs zero quota, and 429s back off honouring Retry-After for up to 8 attempts then fail loud rather than retrying forever. Deliberate scope cut: crop_export.py's exact-boxes-from-cached-metadata optimisation was not ported because its hardcoded ROOT_ID and FILE_KEY made it project-specific. UNTESTED: every network path, because no FIGMA_TOKEN exists in this environment. Offline logic was verified for real, including the depth-cap guard firing and the full slice algorithm cutting 2 tiles from a synthetic two-frame PNG.

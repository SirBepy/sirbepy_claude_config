...
- Advanced in /mega-todos wave 2, commit `414efa6`. Item 1 is DONE: `skills/e2e/SKILL.md`'s "anything else" row now tells the skill to ask how it would observe the behaviour with no test suite at all and to drive the real system by hand, rather than reporting that no e2e path exists. Items 2 and 3 both got verdicts, recorded here rather than built:
  - **Item 2, an analytical Q&A skill: SKIP.** Receipt: `memory/reference_hermes_agent_verdict.md` records a real open-ended analytical question resolved cleanly by ordinary subagent research, with no missing structured-verification capability surfacing. Todo 444 itself was a second instance of the same shape and was resolved the same way, by targeted checks rather than a multi-step skill.
  - **Item 3, per-launch MCP scoping: SKIP, but the flags are real.** `claude --help` confirms `--mcp-config <configs...>` and `--strict-mcp-config` exist in this harness version. The existing claude/claude-light/claude-mid/claude-heavy PowerShell wrappers only vary model and effort env vars and none pass MCP flags, so adopting this means new wrappers, not a tweak.
Remaining: the acceptance item "anything adopted has been used once on real work" is moot, since nothing was adopted. Close this out once the two SKIP verdicts above are accepted.
- All three items already carry recorded adopt/skip verdicts with receipts (2026-09-01 note); the remaining acceptance item is explicitly moot since nothing was adopted. Dropped via /mega-todos 2026-09-04.

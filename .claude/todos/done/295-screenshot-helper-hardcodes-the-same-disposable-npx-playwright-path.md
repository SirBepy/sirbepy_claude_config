...
- Done 2026-08-13. Chose the third option the todo weighed: lifted getChromium()/findInNpxCache() out of hs_common.cjs into a neutral shared module at skills/_shared/playwright-resolve.cjs, so neither skill depends on a path inside the other. hs_common.cjs requires it and re-exports getChromium for compatibility; hs_preflight.cjs and hs_weekshot.cjs needed no change since they never imported getChromium directly, and both were re-verified to still start. screenshot-helper.cjs resolves chromium lazily inside the async run functions so usage errors still work without playwright installed. Verified both ways for real: with the npx cache present a screenshot captures and renders correctly; with the cache renamed away it fails with the actionable message naming the two fix commands, not a raw MODULE_NOT_FOUND. No hash-pinned _npx path remains anywhere under skills/.

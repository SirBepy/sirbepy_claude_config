...
- The repo's large unpushed backlog is not itself a problem to fix here; it is the condition that
  exposes this one.
- 9d6ce7c: unpushed-overlap check now uses file-level intersection only as a prefilter, then confirms via git blame on the pending hunks. Unattended behaviour documented in the same file.

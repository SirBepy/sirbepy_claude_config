...
- Completed via /auto-do-todos 2026-08-15: added consume_fresh_marker(marker_dir, glob_pattern, freshness_seconds, exclude_prefix=None) -> bool to hooks/_hooklib.py, centralising the deliberate OSError swallow with its reasoning, and collapsed the six-line copy in pr-guard, commit-guard and shortcut-create-guard to one call each. Chose return-boolean over sys.exit inside the helper so _hooklib stays testable and each guard keeps its exit visible at the call site. shortcut-create-guard still re-exports oldest_fresh_marker because test_shortcut_create_guard.py reaches it by attribute on the guard module. All five hook suites pass before and after; pr-guard and commit-guard exercised by hand (allow consumes the marker, no marker blocks with exit 2 and unchanged messages).

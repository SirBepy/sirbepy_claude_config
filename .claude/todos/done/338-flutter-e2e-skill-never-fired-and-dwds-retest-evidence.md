...
- Completed via /auto-do-todos 2026-08-15. Part 1: refs/delegation-doctrine.md now requires every dispatch to name the matching skill file paths rather than hand-copying its rules, written as a general rule since subagents cannot invoke skills at all. Part 2: refs/flutter-web-playwright.md records the 2026-08-14 zng-admin re-test (warm-process hang ~4.5 min per route, cold restart never painted in 180s despite a correct 8.7 KB DWDS bootstrap stub, release worked every time, ~2h plus a killed agent lost) alongside the 2026-08-10 observation, with an explicit note that the two points together mean the variable is not understood, not that debug is safe. Mode A needed no change: it already builds release unconditionally, which is the strictest reading of the evidence.

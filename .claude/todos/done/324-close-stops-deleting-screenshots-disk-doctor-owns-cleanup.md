...
- Completed via /auto-do-todos 2026-08-15: /close no longer deletes screenshots at all. Phase 3 step 3 is now a count-only "Screenshot summary", Phase 4 reports "N screenshots written to <dir>" instead of "N cleaned", and CLAUDE.md:84 reframes the per-session subfolder as a browsable-per-chat unit that /disk-doctor ages out. disk-doctor gained a recursive .for_bepy/screenshots/ scan in both windows.md and macos.md plus a KNOWN SAFE-TO-DELETE entry at 30 days, and stayed strictly advise-only per its own invariant, so the todo "owns cleanup" is satisfied as owning the reporting decision, not as an auto-deleter. close/light.md needed no change, it never had its own screenshot step and inherits the fixed Phase 4.

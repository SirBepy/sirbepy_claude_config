...
- Shipped 2026-08-22, manual-only per Joe. skills/heal-skill/{SKILL.md,references/confusion-patterns.md} plus a per-skill heal-log convention at skills/<name>/heal-log.md that the skill reads FIRST, so a second heal of the same symptom is recognised as the previous patch having failed. Trigger: MANUAL, stated in the skill itself with the reason (the delegation doctrine watchdog fired 0 times in 8 dispatches and was deleted, not reworded). Both mechanical triggers considered and rejected in writing: a repeat-invocation counter cannot tell a long correct run from a loop, and a UserPromptSubmit correction-phrase matcher would run on every prompt in every project for a case that arrives a few times a month. TAXONOMY REWRITTEN, which is the substantive finding: the 5 named done/ todos were classified against the upstream 7 patterns and ZERO fit cleanly, one partially (251 host-impossibility, 245 unowned inter-skill conflict, 261 missing runtime branch, 337 wrong premise about intent, 230 internal double standard). Four of the upstream 7 (missing examples, implicit file refs, missing variables, wrong tool) appear nowhere in 258 archived skill-improvement todos, because they diagnose a rigid IF/THEN/Variables house style these skills do not use. Replacement taxonomy is 8 patterns, each grounded in named real todos, with P1 (the rule depends on being remembered) the largest class by far. Ran end to end on a real case: /rate-it not ordering its lift bullets by score, 2 of 3 recorded runs with two or more scored bullets. Classified P1, produced a literal one-line patch at SKILL.md:107 (move the rule to its point of use, explicitly NOT a reword), and STOPPED at the approval gate, which is the gate working rather than a shortfall. A second finding that run, the 2-4 vs cap-at-3 contradiction, was filed as todo 475 instead of folded in, per the skill own one-cause-per-run rule. Hands off to 422 harness for verification and says plainly when a patch is unmeasured. Description is 188 chars, over the 120 budget, which does not apply: disable-model-invocation keeps it out of the always-loaded listing entirely.

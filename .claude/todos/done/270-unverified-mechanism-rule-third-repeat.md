...
- Done 2026-08-13, both halves per Joe. Half A: the rule was NEVER global. It lived only in claude_usage_in_taskbar's project memory with 6 stacked repeats, which is plausibly why it kept recurring: it protected one project, not the tree. Promoted to global CLAUDE.md Execution Discipline, targeting the actual failure mechanism (a hedged noun next to an unhedged 'so Y happens' clause still reads as asserted) and giving a cheap out (write UNVERIFIED plus what you would check). Half B: hook spike says DO NOT INSTALL. Stop/SubagentStop do carry last_assistant_message (verified against live docs), so the text IS visible, but the regex prototype flagged 4 of 6 innocent sentences and missed 3 of 5 real incidents. The label lives in the writer's epistemic state, not the sentence's surface form. Prototype left uninstalled at hooks/EXPERIMENTAL-unverified-mechanism-detector.py. Next spike, if any, should test an LLM judge, not more regex.

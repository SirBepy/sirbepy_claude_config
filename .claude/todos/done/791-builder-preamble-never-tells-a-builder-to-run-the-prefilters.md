...
- Done 2026-08-26: refs/builder-preamble.md's static block now carries the prefilter-gate.sh step with the trim/fix/STOP-on-secret split and the exit-2 case. Decisions recorded in the file: unconditional body text, not a placeholder (an empty diff makes the gate a no-op, so a read-only dispatch pays nothing, and a per-dispatch judgment call is the drift mechanism this file exists to remove); the READ-ONLY DISPATCH marker exempts only the screenshot id; no fourth marker in dispatch-preamble-guard.py. Script path absolute, file arguments either form, since 447 made the gate resolve the target repo from its first path argument. refs/delegation-doctrine.md's three restated bullets collapsed to one pointer at the block, the same treatment it already gives the run_in_background rules.

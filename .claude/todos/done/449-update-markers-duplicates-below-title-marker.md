...
- Done 2026-08-26: Get-HeaderMarkers finds an above-title and a below-title marker bounded by the first '## ' heading; a below-title marker is always cut and the write always lands above the title, so the end state satisfies Step 5's gate in every case. Baseline is the chronologically older marker. Line endings are derived from the file instead of a hardcoded CRLF. Verified on five fixtures: below-only 3 to 4 then 5, both-markers 9 to 10 then 11, above-only byte-identical to the pre-change script, no-marker inserts once, and a file quoting the format inside '## Notes' is untouched. All five end with exactly one header marker above the title and stay LF-only. Step 5's gate gained a resulting-state check scoped to the header region, plus a carve-out for the relocation deletion.

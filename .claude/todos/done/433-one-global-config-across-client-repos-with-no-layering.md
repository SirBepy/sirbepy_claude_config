...
- Advanced in /mega-todos wave 2, commit `0e48e6e`, NOT finished. Three project-specific facts were stripped from global CLAUDE.md, each because an authoritative source already held it: the zng-app/zng-biller shared-index example (covered by `refs/builder-preamble.md`'s placeholder table), the gh-account mapping (covered by `hooks/gh-account-switch.sh`'s own header), and the Fibo lucide-react icon opt-out (the general rule already says a project opts out in its own CLAUDE.md). Remaining, and genuinely outside this repo: the acceptance item asking for one rule migrated and proven to apply in its repo but not another requires editing a client repo's own CLAUDE.md. That has to happen from a session in that repo, not from here.
- Global-side work already landed: CLAUDE.md carries zero client-repo names (verified /cleanup-todos 2026-09-04). The one open acceptance item edits a client repo's own CLAUDE.md, which is unreachable from this repo. Dropped via /mega-todos 2026-09-04.

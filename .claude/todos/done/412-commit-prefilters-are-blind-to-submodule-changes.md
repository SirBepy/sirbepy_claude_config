...
- Done 2026-08-26: prefilter-gate.sh groups every path argument by its own resolved repo root, rebuilds a foreign-repo path relative to that root via rev-parse --show-prefix, and runs each wrapped script once per group, unioning output and keeping the worst exit code. Detect-and-re-run, no case needed the detect-and-refuse fallback. Verified on a scratch parent plus real submodule: a planted credential in a submodule file was invisible to the pre-412 gate (exit 0, no output) and now exits 1 naming the file, and a mixed invocation reports a parent em dash and a submodule credential together. Seven non-submodule shapes (clean single, clean multi, zero args, --range, explicit -C, bad path, bad --repo) plus a real single-repo hit are byte-identical to the pre-412 script. The wrapped scripts needed no change; the bug was entirely in the gate's argument handling. mega-todos' injected block was calling the three scripts DIRECTLY, bypassing the gate and comment-tense.sh entirely, so it now routes through the gate and documents comment-tense's treatment plus exit 2.

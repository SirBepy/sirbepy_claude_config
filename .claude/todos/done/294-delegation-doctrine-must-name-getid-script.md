...
- The project-side half of this is already recorded in that project's memory
  ...
  lands.
- Done 2026-08-13. All THREE mentions of the screenshot path in refs/delegation-doctrine.md now name rename-session.ps1 -GetId (.sh --get-id on Unix), say to resolve it ONCE per session and reuse it, and state the prohibition explicitly: never from a process-tree walk, never a hand-picked folder name, with the reason (/close can only delete its own authoritative subfolder, so a wrong id is permanently un-cleanable and may collide with a live session's). Approach item 3 was implemented rather than just considered: /close Phase 3 step 3 now counts and REPORTS other-session subfolders it cannot claim instead of silently leaving them, and Phase 4's counter line reports them. That is what would have surfaced the original incident on its first occurrence instead of its second.

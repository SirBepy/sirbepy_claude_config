...
- Done 2026-08-16, commit 06d3404. adb-drive.ps1 gained Get-OrientationInfo: when the screenshot and wm size differ by an exact WxH transposition, no scaling is applied. Portrait math untouched. No Android device was attached, so this is offline-verified only: a PowerShell parse check, plus the four dimension cases (including the todo's exact 2400x1080-vs-1080x2400 bug case) exercised ad hoc against Get-OrientationInfo in-session. Correction added 2026-08-16 by the run's closing code-check: those cases were NEVER COMMITTED as a test file, and this repo has no PowerShell test harness, so there is no durable regression test for this fix. The only standing evidence is the parse check and inspection. Not yet re-run live.

...
- Fixed 2026-08-25: header now names all four consuming hooks (shortcut-create, linear-create, linear-update, shortcut-mutation) with what each gates, plus which marker glob each accepts. Verified against MARKER_GLOBS in each hook; .shortcut-marker* is documented as the legacy name per shortcut-create-guard.py:35.

...
- Fixed 2026-08-25. Shared clause 'When the Agent tool is unavailable' added to refs/delegation-doctrine.md under the 90/10 rule, so every adopting skill inherits it without a copy: do the step inline, name the substitution in the summary, never call the tool anyway and never silently drop the step. All three call sites now point at it with the per-site decision the todo asked for - auto-do-todos Step 4 inline (todos already read), Step 9 inline (the fan-out buys context economy not capability), and code-check 4.2 bounded, since keeping doc bytes out of context IS its purpose: inline under ~10 files, skip-with-reason above. This session is itself an instance, its system prompt carries the same do-not-call-AgentTool directive.
